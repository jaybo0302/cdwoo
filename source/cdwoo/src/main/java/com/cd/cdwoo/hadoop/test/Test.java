...
//        }
      ...
      ///////////////////////////================activity reduce==================///////////////////////
        ...
        long totalTime = 0;
        long acTime = 0;
        String recordLocation = valueMap.get(list.get(0)).split(",")[0]+"," + valueMap.get(list.get(0)).split(",")[1];
        for(long t : list) {
          if (t < list.get(list.size()-1)) {
            ...
            if (recordLocation.equals(currentLocation)) {
              ...
              if (endTime - beginTime > 0) {
                ...
                recordLocation = valueMap.get(list.get(list.indexOf(t)+1)).split(",")[0]+","+valueMap.get(t).split(",")[1];
              }
            }
          } else {
            //如果遍历到最后一条
            //判断当前位置是否与上一条一样，如果一样，计算时间，如果不一样，将上一条记录之前的记录做统计，如果活动时间不为0，写入一条到hdfs
            String currentLocation = valueMap.get(t).toString().split(",")[0]+","+valueMap.get(t).split(",")[1];
            if (recordLocation.equals(currentLocation)) {
              if (t - beginTime > 0) {
                acTime = (t - beginTime);
                totalTime = totalTimeMap.get(recordLocation) == null ? 0 : totalTimeMap.get(recordLocation);
